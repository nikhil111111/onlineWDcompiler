...
//     } catch (error) {
//         return res.status(500).send({ message: "Error Saving your code", error });
//     }
// };
export const saveCode = async (req: AuthRequest, res: Response) => {
    ...
    try {
        ...
        console.log("New code ID:", newCode._id);

        if (isAuthenticated && user) {
            user.savedCodes.push(newCode._id);
            console.log("Before saving user, savedCodes:", user.savedCodes);

            await user.save();
            console.log("After saving user, savedCodes:", user.savedCodes);
        }
